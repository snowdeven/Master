...
N_c=2
N_r=2
N=N_c*N_r #numbers of oscillators 
tf=10 #time of the simulation 
t0=0 #time of the beginning of the simultaion
t=0 
nb=100# number of steps
kappa= 10#coefficient of coupling
m = 1
sigma=0.5
Open=False
...
for i in range(N):
    ...
    for j in l:
        if Open == False:
            ...
            if j >= 0 and j <= N-1 and i!=j:
               K[i,j] = kappa
           
               
      
               
# declaring the delay matrix

tausup=np.zeros((N,N)) #matrix of zeros
for i in range(N):
    for j in range(N):
        if i >= j+1  and i<= j+m*2: #all matrix
            tausup[i][j] = abs(i-j)
tau = np.transpose(tausup)+tausup #transpose to have ij = ji

#declaring the dephasing matrix

alpha=2*np.pi*np.random.rand(N,N)
# declaring the white noise matrix


eta=np.zeros((N,nb))
for i in range(nb):
    eta[:,i] = np.random.normal(0,sigma/h,N)
...
def F(x,Y,i,jose):
    #this is te definition of the differentiel equation
    ...
    for j in range(N):
        Z=jose - int(tau[i,j])
        non_coupling += K[i,j] * np.sin(Y[j,Z]-x+alpha[i,j])
       
    
    return (W[i] + (1/N)*(non_coupling)+eta[i,jose])





def RK4_t(Y,jose,i,h):
        #definition of runge-kutta order 4 to solve the differential equation
        K1=F(Y[i][jose],A,i,jose)
        K2=F(Y[i][jose]+K1*(h/2),A,i,jose)
        K3=F(Y[i][jose]+K2*(h/2),A,i,jose)
        K4=F(Y[i][jose]+K3*h,A,i,jose) 
        return (Y[i][jose] +( K1 + 2*K2 + 2*K3 +K4 )*(h/6))
